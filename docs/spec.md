# http-adaptor MCP Specification
...
flowchart TD
    ...
    mgmt[tools.py<br/>Management tools]
...
3. Request is dispatched with retry and timeout resolved from explicit tool values or app defaults
...
### 5.3 Timeout resolution

Request timeout is resolved from the explicit tool `timeout_seconds` value when present, otherwise from the app-level `timeout_seconds` default.

### 5.4 LLM-friendly errors
...
- OpenAPI v2 / Swagger 2.0 import support.
